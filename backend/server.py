...
def public_user(u: Optional[dict]) -> Optional[dict]:
    ...
    badge_expiry = parse_ts(data.get("badge_expires_at"))
    if badge_expiry and badge_expiry <= now_utc():
        data["badge_type"] = None
        data["badge_icon"] = None
        data["badge_expires_at"] = None

    anim_expiry = parse_ts(data.get("profile_animation_expires_at"))
    if anim_expiry and anim_expiry <= now_utc():
        data["profile_animation"] = None
        data["profile_animation_speed"] = None
...
# ── Notification helper ────────────────────────────────────────────────────────
MESSAGE_NOTIF_COOLDOWN = timedelta(hours=12)
...
async def _push_notification(to_user: str, kind: str, data: dict):
    ...
    if kind == "message":
        ...
        if sender_id:
            cutoff = ts(now_utc() - MESSAGE_NOTIF_COOLDOWN)
            existing = await run(
                lambda: sb.table("notifications")
                .select("notif_id")
                .eq("user_id", to_user)
                .eq("kind", "message")
                .contains("data", {"from": sender_id})
                .gte("created_at", cutoff)
                .limit(1)
                .execute()
            )
            suppress_in_app = bool(existing.data)

    if not suppress_in_app:
        notif = {
...
async def list_messages(
    ...
    # Filter deleted_for in Python, mark read (batch update)
    visible = []
    unread_updates: dict = {}
    read_broadcasts = []
    for msg in msgs:
        ...
        if msg["sender_id"] != user["user_id"] and user["user_id"] not in (msg.get("read_by") or []):
            ...
            unread_updates[msg["message_id"]] = (msg["sender_id"], new_rb)
            msg["read_by"] = new_rb
            read_broadcasts.append({
                "type": "message_read",
                "conversation_id": conversation_id,
    ...
    if unread_updates:
        tasks = []
        for mid, (sender_id, new_rb) in unread_updates.items():
            tasks.append(run(lambda: sb.table("messages").update({"read_by": new_rb}).eq("message_id", mid).execute()))
            tasks.append(broadcast_to_user(sender_id, {
                "type": "message_read",
                "conversation_id": conversation_id,
                "message_id": mid,
                "read_by": new_rb,
...
async def list_notifications(user=Depends(current_user)):
    ...
    if sender_ids:
        ur = await run(lambda: sb.table("users")
            .select(
                "user_id,username,display_name,profile_picture,badge_type"
                f"{premium_cols_suffix()}"
            )
    ...
    for n in notifications:
        ...
        n["sender"] = public_user(users_map.get(sender_id))
...
from frame_ids import FRAME_IDS as _GENERATED_FRAME_IDS

VALID_PROFILE_FRAMES = set(_GENERATED_FRAME_IDS)
VALID_ACHIEVEMENTS = {"bronze", "silver", "gold", "platinum", "diamond"}
VALID_PROFILE_ANIMATIONS = {
    "off", "glow", "pulse", "rotate", "sparkle", "aura", "orbit", "particles",
    "shimmer", "rainbow", "neon", "fire", "lightning", "frost", "magic",
    "butterfly", "sakura", "roses", "golden", "snow", "hearts", "stars",
    "galaxy", "meteor", "ripple", "crystal", "phoenix", "dragon", "angel",
    "shadow", "holographic", "custom",
}
VALID_ANIMATION_SPEEDS = {"slow", "normal", "fast"}
VALID_ANIMATION_INTENSITIES = {"low", "medium", "high"}

PREMIUM_MIGRATION_SQL = (
    "ALTER TABLE users ADD COLUMN IF NOT EXISTS badge_icon TEXT DEFAULT NULL;\n"
    "ALTER TABLE users ADD COLUMN IF NOT EXISTS badge_expires_at TIMESTAMPTZ DEFAULT NULL;\n"
    "ALTER TABLE users ADD COLUMN IF NOT EXISTS profile_frame TEXT DEFAULT NULL;\n"
    "ALTER TABLE users ADD COLUMN IF NOT EXISTS achievement_level TEXT DEFAULT NULL;\n"
    "ALTER TABLE users ADD COLUMN IF NOT EXISTS profile_animation TEXT DEFAULT NULL;\n"
    "ALTER TABLE users ADD COLUMN IF NOT EXISTS profile_animation_speed TEXT DEFAULT NULL;\n"
    "ALTER TABLE users ADD COLUMN IF NOT EXISTS profile_animation_intensity TEXT DEFAULT NULL;\n"
...
def premium_cols_select() -> str:
    return (
        "badge_icon,badge_expires_at,profile_frame,achievement_level,profile_animation,"
        "profile_animation_speed,profile_animation_intensity,profile_animation_expires_at"
        if _PREMIUM_COLS else ""
    )


def premium_cols_suffix() -> str:
    return (
        ",badge_icon,badge_expires_at,profile_frame,achievement_level,profile_animation,"
        "profile_animation_speed,profile_animation_intensity,profile_animation_expires_at"
        if _PREMIUM_COLS else ""
...
async def admin_list_users(
    ...
    query = sb.table("users").select(
        "user_id,email,username,display_name,profile_picture,bio,badge_type"
        f"{premium_cols_suffix()},"
        "online,created_at,email_verified"
    )
    if q:
...
async def admin_get_user(
    ...
    r = await run(lambda: sb.table("users").select(
        ...
        "badge_type"
        f"{premium_cols_suffix()},"
        "online,last_seen,created_at,email_verified,is_private,provider,"
...
class PremiumBody(BaseModel):
    ...
    profile_animation: Optional[str] = None
    profile_animation_speed: Optional[str] = None
    profile_animation_intensity: Optional[str] = None
    profile_animation_expires_at: Optional[str] = None


@api.get("/admin/users/{user_id}/premium")
async def admin_get_premium(
    user_id: str,
    _admin: dict = Depends(require_admin),
):
    """Get a user's premium setup (badge, custom icon, frame, achievement, animation)."""
    if not _PREMIUM_COLS:
        raise HTTPException(
            status_code=503,
            detail=f"Premium columns not migrated yet. Run this SQL in Supabase:\n{PREMIUM_MIGRATION_SQL}",
        )
    r = await run(lambda: sb.table("users").select(
        "user_id,badge_type,badge_icon,badge_expires_at,verified_since,profile_frame,achievement_level,"
        "profile_animation,profile_animation_speed,profile_animation_intensity,profile_animation_expires_at"
    ).eq("user_id", user_id).execute())
    if not r.data:
        raise HTTPException(status_code=404, detail="User not found")
    return {"premium": r.data[0]}


@api.put("/admin/users/{user_id}/premium")
async def admin_set_premium(
    user_id: str,
    body: PremiumBody,
    _admin: dict = Depends(require_admin),
):
    """Assign or remove a user's premium badge, custom icon, profile frame, achievement level and animation."""
    if not _PREMIUM_COLS:
        raise HTTPException(
            status_code=503,
            detail=f"Premium columns not migrated yet. Run this SQL in Supabase:\n{PREMIUM_MIGRATION_SQL}",
        )
    if body.badge_type is not None and body.badge_type not in VALID_BADGE_TYPES:
        raise HTTPException(status_code=400, detail=f"Invalid badge type. Valid: {sorted(VALID_BADGE_TYPES)}")
    if body.profile_frame is not None and body.profile_frame not in VALID_PROFILE_FRAMES:
        raise HTTPException(status_code=400, detail="Invalid profile frame. Valid frames: %d available (e.g. gold, emerald, gold-apex)." % len(VALID_PROFILE_FRAMES))
    if body.achievement_level is not None and body.achievement_level not in VALID_ACHIEVEMENTS:
        raise HTTPException(status_code=400, detail=f"Invalid achievement level. Valid: {sorted(VALID_ACHIEVEMENTS)}")
    if body.profile_animation is not None and body.profile_animation not in VALID_PROFILE_ANIMATIONS:
        raise HTTPException(status_code=400, detail=f"Invalid profile animation. Valid: {sorted(VALID_PROFILE_ANIMATIONS)}")
    if body.profile_animation_speed is not None and body.profile_animation_speed not in VALID_ANIMATION_SPEEDS:
        raise HTTPException(status_code=400, detail=f"Invalid animation speed. Valid: {sorted(VALID_ANIMATION_SPEEDS)}")
    if body.profile_animation_intensity is not None and body.profile_animation_intensity not in VALID_ANIMATION_INTENSITIES:
        raise HTTPException(status_code=400, detail=f"Invalid animation intensity. Valid: {sorted(VALID_ANIMATION_INTENSITIES)}")
    if body.badge_icon is not None and len(body.badge_icon) > 40:
        raise HTTPException(status_code=400, detail="Badge icon name is too long")

    expires = parse_ts(body.badge_expires_at) if body.badge_expires_at else None
    if body.badge_expires_at and expires is None:
        raise HTTPException(status_code=400, detail="badge_expires_at must be a valid ISO timestamp")

    anim_expires = parse_ts(body.profile_animation_expires_at) if body.profile_animation_expires_at else None
    if body.profile_animation_expires_at and anim_expires is None:
        raise HTTPException(status_code=400, detail="profile_animation_expires_at must be a valid ISO timestamp")

    ur = await run(lambda: sb.table("users")
        .select("user_id,badge_type,verified_since")
        .eq("user_id", user_id).execute())
    if not ur.data:
        raise HTTPException(status_code=404, detail="User not found")

    current = ur.data[0]
    verified_since = current.get("verified_since")
    if body.badge_type is not None:
        if not verified_since:
            verified_since = ts(now_utc())
    else:
        verified_since = None

    upd = {
        "badge_type": body.badge_type,
        "badge_icon": (body.badge_icon or "").strip() or None,
        "badge_expires_at": ts(expires) if expires else None,
        "verified_since": verified_since,
        "profile_frame": (body.profile_frame or "").strip() or None,
        "achievement_level": (body.achievement_level or "").strip() or None,
        "profile_animation": (body.profile_animation or "").strip() or None,
        "profile_animation_speed": (body.profile_animation_speed or "").strip() or None,
        "profile_animation_intensity": (body.profile_animation_intensity or "").strip() or None,
        "profile_animation_expires_at": ts(anim_expires) if anim_expires else None,
    }
    await run(lambda: sb.table("users").update(upd).eq("user_id", user_id).execute())

    return {
        "ok": True,
        "badge_type": upd["badge_type"],
        "badge_icon": upd["badge_icon"],
        "badge_expires_at": upd["badge_expires_at"],
        "verified_since": upd["verified_since"],
        "profile_frame": upd["profile_frame"],
        "achievement_level": upd["achievement_level"],
        "profile_animation": upd["profile_animation"],
        "profile_animation_speed": upd["profile_animation_speed"],
        "profile_animation_intensity": upd["profile_animation_intensity"],
        "profile_animation_expires_at": upd["profile_animation_expires_at"],
    }
...
async def startup():
    ...
    # Ensure premium columns exist (badge_icon, badge_expires_at, profile_frame,
    # achievement_level, profile_animation, speed, intensity, expires_at)
    global _PREMIUM_COLS
    try:
        await run(lambda: sb.table("users").select(
            "badge_icon,badge_expires_at,profile_frame,achievement_level,profile_animation,"
            "profile_animation_speed,profile_animation_intensity,profile_animation_expires_at"
        ).limit(1).execute())
        _PREMIUM_COLS = True
        logger.info("premium columns ✅")
    except Exception:
        _PREMIUM_COLS = False
        logger.warning(
            "⚠️  premium columns missing. Run in Supabase SQL Editor:\n%s",
            PREMIUM_MIGRATION_SQL,
        )
...
_GEMINI_MODEL = os.environ.get("GEMINI_MODEL", "gemini-3.5-flash").strip() or "gemini-3.5-flash"
