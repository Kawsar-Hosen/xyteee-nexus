...
export default function StoryViewer() {
  ...
  const sendReply = async () => {
    ...
    try {
      ...
      /* Send the story itself as an image message first (Instagram-style reply preview) */
      if (s.kind === "image" && s.media) {
  ...
  const reactToStory = async (emoji: string) => {
    ...
    /* Heart love reaction special handling */
    if (emoji === "❤️") {
      /* Scale up heart with burst effect */
      const burstAnim = new Animated.Value(1);
      Animated.sequence([
        Animated.timing(burstAnim, { toValue: 1.45, duration: 150, useNativeDriver: true }),
        Animated.timing(burstAnim, { toValue: 1, duration: 200, useNativeDriver: true }),
      ]).start();
    }
          ...
          <TouchableOpacity onPress={() => router.push(`/user/${author?.user_id}`)} style={{ flexDirection: "row", alignItems: "center", flex: 1 }}>
            ...
            <View style={{ marginLeft: 10, flex: 1 }}>
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                ...
                <VerifiedBadge badgeType={author?.badge_type} badgeIcon={author?.badge_icon} badgeExpiresAt={author?.badge_expires_at} size={16} />
              </View>
              ...
              renderItem={({ item }) => (
                <View style={styles.viewerRow}>
                  <Avatar
                    ...
                    size={46}
                    frame={item.user?.profile_frame}
                    achievement={item.user?.achievement_level}
                    animation={item.user?.profile_animation}
                    animationSpeed={item.user?.profile_animation_speed}
                    animationIntensity={item.user?.profile_animation_intensity}
                  />
                  ...
                  <View style={styles.viewerInfo}>
                    <View style={styles.viewerNameRow}>
                      ...
                      <VerifiedBadge badgeType={item.user?.badge_type} badgeIcon={item.user?.badge_icon} badgeExpiresAt={item.user?.badge_expires_at} size={16} />
                    </View>
