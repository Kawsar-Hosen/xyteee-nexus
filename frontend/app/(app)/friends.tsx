            ...
            <View style={styles.row}>
              <TouchableOpacity style={{ flexDirection: "row", alignItems: "center", flex: 1 }} onPress={() => router.push(`/user/${item.user_id}`)}>
                <Avatar
                  ...
                  achievement={item.achievement_level}
                  animation={item.profile_animation}
                  animationSpeed={item.profile_animation_speed}
                  animationIntensity={item.profile_animation_intensity}
                  online={item.online}
                ...
                <VerifiedBadge badgeType={item.badge_type} badgeIcon={item.badge_icon} badgeExpiresAt={item.badge_expires_at} size={16} />
              </View>
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: DOCK_PAD, paddingTop: spacing.md }}>
          ...
          {incoming.map((r) => (
            <View key={r.request_id} style={styles.row}>
              <Avatar
                uri={r.user?.profile_picture}
                name={r.user?.display_name}
                size={44}
                frame={r.user?.profile_frame}
                achievement={r.user?.achievement_level}
                animation={r.user?.profile_animation}
                animationSpeed={r.user?.profile_animation_speed}
                animationIntensity={r.user?.profile_animation_intensity}
              />
              <View style={{ flex: 1, marginLeft: 14 }}>
                <View style={{ flexDirection: "row", alignItems: "center" }}>
                  <NxText variant="titleSm">{r.user?.display_name}</NxText>
                  <VerifiedBadge badgeType={r.user?.badge_type} badgeIcon={r.user?.badge_icon} badgeExpiresAt={r.user?.badge_expires_at} size={16} />
                </View>
